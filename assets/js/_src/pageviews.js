...
function countPV(path, rows) {
	...
	var fileName = path
		.replace(/\/posts\//g, '')
		.replace(/\//g, '.html'); /* e.g. post-title.html */
	var count = 0;

	var _v2_url = path.replace(
		/posts\//g,
		''
	); /* the v2.0+ blog permalink: "/post-title/" */
	...
	for (var i = 0; i < rows.length; ++i) {
		...
		if (
			...
			gaPath.slice(gaPath.lastIndexOf('/') + 1) ===
				fileName
		) {
...
var PvCache = (function () {
	...
	return {
		...
		getAllPagevies: function () {
			return PvCache.getData().totalsForAllResults[
				'ga:pageviews'
			];
		},
...
function fetchProxyPageviews() {
	...
	$.ajax({
		...
		error: function (jqXHR, textStatus, errorThrown) {
			console.log(
				'Failed to load pageviews from proxy server: ' +
					errorThrown
			);
		},
	});
...
$(function () {
	if ($('.pageviews').length > 0) {
		let originPvData = pageviews
			? JSON.parse(pageviews)
			: undefined;
